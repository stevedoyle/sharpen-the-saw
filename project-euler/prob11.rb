...
grid = [%w{02 22 97 38 15 00 40 00 75 04 05 07 78 52 12 50 77 91 08 },
...
# horizontal
grid.each do |row|
  (0..(row.length-4)).each {|i| sums << row[i...(i+4)].mul}
end

puts "#{sums.length} \t #{sums.max}"

# vertical
(0...grid[0].length).each do |col|
  (0..(grid.length-4)).each {|row| sums << (grid[row][col] * grid[row+1][col] * grid[row+2][col] * grid[row+3][col]) }
end
...
(0...(grid[0].length-4)).each do |col|
  (0..(grid.length-4)).each {|row| sums << (grid[row][col] * grid[row+1][col+1] * grid[row+2][col+2] * grid[row+3][col+3]) }
end

puts "#{sums.length} \t #{sums.max}"

# other diagonal
(3...(grid[0].length)).each do |col|
  (0..(grid.length-4)).each {|row| sums << (grid[row][col] * grid[row+1][col-1] * grid[row+2][col-2] * grid[row+3][col-3]) }
end
